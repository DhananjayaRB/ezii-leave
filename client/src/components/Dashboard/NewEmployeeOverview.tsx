...
export default function NewEmployeeOverview() {
  ...
  console.log(
    "🚨 [NewEmployeeOverview] CRITICAL DEBUG - Loading component for user:",
    currentUserId,
  );
  console.log(
  ...
  console.log("🚨 [NewEmployeeOverview] Expected org_id for correct data: 13");

  // Force org_id to 13 for correct data (same as Leave Applications)
  React.useEffect(() => {
    const currentOrgId = localStorage.getItem("org_id");
    if (currentOrgId !== "13") {
      console.log(
        "🚨 [NewEmployeeOverview] Switching from org_id",
        currentOrgId,
        "to 13 for correct data",
      );
      localStorage.setItem("org_id", "13");
      // Force page refresh to use correct org context
  ...
  const calculateSophisticatedMetrics = () => {
    ...
    const variantsArray = Array.isArray(leaveVariantsData)
      ? leaveVariantsData
      : [];

    console.log(
      "🎯 [NewEmployeeOverview] EXACT Leave Applications calculation:",
      {
        currentUserId,
    ...
    const assignedVariantIds = currentUserAssignments.map(
      (a: any) => a.leaveVariantId,
    );
    ...
    availableLeaveVariants.forEach((variant: any) => {
      ...
      const transactions = Array.isArray(transactionsArray)
        ? transactionsArray.filter((t: any) => t.leaveVariantId === variant.id)
        : [];
    ...
    const allUserTransactions = (transactionsArray as any[]).filter(
  ...
  const generateUsageTrends = () => {
    ...
    console.log(
      "[UsageTrends] DEBUG - Total leave requests:",
      leaveRequests.length,
    );
    console.log(
      "[UsageTrends] DEBUG - Approved leaves:",
      approvedLeaves.length,
    );
    console.log(
      "[UsageTrends] DEBUG - Pending leaves:",
    ...
    console.log(
      "[UsageTrends] DEBUG - Using leaves for trends:",
    ...
    return months.map((month, index) => {
      ...
      console.log(
        `[UsageTrends] ${month} (${index}) - Found ${monthLeaves.length} leaves:`,
        monthLeaves.map((l) => ({
          startDate: l.startDate,
          workingDays: l.workingDays,
          status: l.status,
      ...
      const totalUsage = monthLeaves.reduce(
        (sum: number, leave: LeaveRequest) => {
          const days = parseFloat(leave.workingDays?.toString() || "0");
          console.log(
            `[UsageTrends] Adding ${days} days from leave:`,
            leave.startDate,
            leave.status,
          );
          return sum + days;
  ...
  console.log(
    "[NewEmployeeOverview] Total approved leaves:",
    approvedLeaves.length,
  );
  console.log(
    "[NewEmployeeOverview] Sample approved leave:",
    approvedLeaves[0],
  );
                ...
                <TabsContent value="leaves" className="space-y-4">
                  {leaveRequests.length > 0 ? (
                    <div className="space-y-3">
                      {leaveRequests
                        .slice(0, 5)
                        .map((request: LeaveRequest, index: number) => (
                          <div
                            key={index}
                ...
                <TabsContent value="pto">
                  <div className="text-center py-8 text-gray-500">
                    {ptoRequests.length === 0
                      ...
                      : `${ptoRequests.length} PTO requests`}
                  </div>
                </TabsContent>
                ...
                <TabsContent value="compoff">
                  <div className="text-center py-8 text-gray-500">
                    {compOffRequests.length === 0
                      ...
                      : `${compOffRequests.length} comp-off requests`}
                  </div>
                </TabsContent>
              </Tabs>
            </CardContent>
