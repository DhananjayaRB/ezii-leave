  ...
  useEffect(() => {
    const loadEmployees = async () => {
      try {
        ...
        if (!token) {
          console.error('[EmployeeAssignment] No JWT token found in localStorage');
          console.log('[EmployeeAssignment] To use external employee data, please:');
          ...
          <table className="w-full">
            <thead className="bg-gray-50 sticky top-0">
              <tr>
                ...
                <th className="p-3 text-left text-sm font-medium text-gray-700">Employee ID</th>
            ...
            <tbody>
              {currentEmployees.map((employee: Employee) => (
                <tr
                  ...
                  className={`border-b hover:bg-gray-50 ${isSelected(employee) ? 'bg-blue-50' : ''}`}
                >
                  <td className="p-3">
                    <input
                      type="checkbox"
                  ...
                  <td className="p-3 text-sm text-gray-900">{employee.employee_number}</td>
                  <td className="p-3 text-sm text-gray-900">{employee.user_name}</td>
                  <td className="p-3 text-sm text-gray-600">{employee.email}</td>
                  <td className="p-3 text-sm text-gray-600">{employee.designation}</td>
                  <td className="p-3 text-sm text-gray-600">{employee.workerType}</td>
                </tr>
              ))}
            </tbody>
          </table>
