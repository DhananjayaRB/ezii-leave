...
export default function Approvals() {
  ...
  const checkBlackoutPeriodConflict = (
    ...
    startDate: string,
    endDate: string,
  ): { hasConflict: boolean; periodName?: string } => {
    ...
    for (const period of blackoutPeriods as any[]) {
      ...
      const hasOverlap =
        requestStart <= blackoutEnd && blackoutStart <= requestEnd;
  ...
  const pendingRequests = currentTabData.filter(
    (req) => req.status === "pending" || req.status === "withdrawal_pending",
  );
  const approvedRequests = currentTabData.filter(
    (req) => req.status === "approved",
  );
  const rejectedRequests = currentTabData.filter(
  ...
  const requestTabs = [
    {
      ...
      active: activeRequestTab === "Leaves",
    },
    {
      id: "PTO",
      label: "PTO",
      count: filteredPtoRequests.length,
      active: activeRequestTab === "PTO",
    },
    {
      id: "CompOff",
      label: "CompOff",
      count: filteredCompOffRequests.length,
      active: activeRequestTab === "CompOff",
    },
  ];
  ...
  const getFilteredRequests = () => {
    ...
    if (searchQuery.trim()) {
      requests = requests.filter((request: any) => {
        ...
        if (activeRequestTab === "PTO") {
          ...
          const leaveTypeObj = (leaveTypes as any[]).find(
    ...
    const sorted = requests.sort((a: any, b: any) => {
      ...
      if (
        a.status === "withdrawal_pending" &&
        b.status !== "withdrawal_pending"
      )
        return -1;
      if (
        b.status === "withdrawal_pending" &&
      ...
      const dateA = new Date(
        a.createdAt || a.startDate || a.requestDate,
      ).getTime();
      const dateB = new Date(
        b.createdAt || b.startDate || b.requestDate,
      ).getTime();
      return dateB - dateA;
    });
  ...
  const approveRequestMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        ...
        description:
          error.message ||
          `Failed to approve ${activeRequestTab.toLowerCase()} request`,
        variant: "destructive",
      });
    },
  });
  ...
  const rejectRequestMutation = useMutation({
    mutationFn: async ({
      ...
      const endpoint = getApprovalEndpoint();
      return apiRequest("POST", `${endpoint}/${requestId}/reject`, {
        reason: reason,
      });
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast({
        ...
        description:
          error.message ||
          `Failed to reject ${activeRequestTab.toLowerCase()} request`,
        variant: "destructive",
      });
    },
  });
  ...
  const getApprovalEndpoint = () => {
    switch (activeRequestTab) {
      case "PTO":
        return "/api/pto-requests";
      case "CompOff":
        return "/api/comp-off-requests";
      default:
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200">
                        ...
                        <th className="text-left py-3 px-4 text-sm font-medium text-gray-500 uppercase tracking-wider w-32">
                          Actions
                        </th>
                      </tr>
                    </thead>
                      ...
                      {paginatedRequests().map((request: any) => {
                        ...
                        if (activeRequestTab === "PTO") {
                          displayInfo = {
                            ...
                            days:
                              request.timeType === "half_day"
                                ? "0.5"
                                : request.timeType === "quarter_day"
                                  ? "0.25"
                                  : request.totalHours
                          ...
                          displayInfo = {
                            ...
                            days:
                              request.amount || request.transferAmount || "1",
                            hasDocuments: request.documentUrl,
                          };
                        } else {
                          ...
                          displayInfo = {
                            ...
                            days:
                              request.workingDays || request.totalDays || "1",
                            hasDocuments:
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-3 px-4 text-gray-700">
                              {displayInfo.startDate === displayInfo.endDate
                            ...
                            <td className="py-3 px-4">
                              {(() => {
                                // Only check blackout conflicts for leave requests, not PTO or CompOff
                                if (activeRequestTab === "Leaves") {
                                  const blackoutCheck =
                                    checkBlackoutPeriodConflict(
                                      request.userId,
                                      request.startDate,
                                      request.endDate,
                                    );
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !rejectionReason.trim() || rejectRequestMutation.isPending
              }
            >
