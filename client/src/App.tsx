...
function Router() {
    ...
    if (currentPath.startsWith("/id/")) {
      ...
      try {
        ...
        if (parts.length === 3) {
          ...
          console.log(
            "JWT processing - preserved user_id:",
            userIdToSet,
            "from JWT:",
  ...
  return (
    <Switch>
      ...
      <Route path="/test">{() => <div>Test route works!</div>}</Route>
      ...
      {!isAuthenticated ? (
        ...
        <>
          ...
          <Route path="/admin-reports/:reportType?" component={AdminReports} />
          <Route
            path="/hr-leave-balance-report"
            component={HRLeaveBalanceReport}
          />
          <Route path="/reports/:reportType?" component={EmployeeReports} />
          <Route
            path="/leave-types/:leaveTypeName"
            component={LeaveTypeVariants}
          />
          <Route path="/loader-demo" component={LoaderDemo} />
          <Route
            path="/time-based-approval-test"
            component={TimeBasedApprovalTest}
          />
        </>
      )}
