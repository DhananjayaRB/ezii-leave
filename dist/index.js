...
__export(schema_exports, {
  ...
  leaveBalanceTransactions: () => leaveBalanceTransactions,
...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    leaveBalanceTransactions = pgTable("leave_balance_transactions", {
      id: serial("id").primaryKey(),
      userId: varchar("user_id").notNull(),
      leaveVariantId: integer("leave_variant_id").notNull(),
    ...
    insertLeaveBalanceTransactionSchema = createInsertSchema(leaveBalanceTransactions);
...
async function migrateEmployeeData(orgId) {
  ...
  for (const mapping of employeeMappings) {
    ...
    try {
      ...
      const transactionUpdateResult = await db.update(leaveBalanceTransactions).set({ userId: user_id }).where(and2(
        eq2(leaveBalanceTransactions.userId, employee_number),
        eq2(leaveBalanceTransactions.orgId, orgId)
      ));
...
var init_employeeMapping = __esm({
  async "server/utils/employeeMapping.ts"() {
    "use strict";
    await init_db();
    init_schema();
  }
...
var DatabaseStorage = class {
  ...
  async getLeaveRequests(userId, orgId) {
    const conditions = [];
  ...
  async getLeaveBalanceTransactions(userId, leaveVariantId, orgId) {
    const transactions = await db.select().from(leaveBalanceTransactions).where(and(
  ...
  async getAllLeaveBalanceTransactions(userId, orgId) {
    ...
    const transactions = await db.select().from(leaveBalanceTransactions).where(whereConditions.length > 0 ? and(...whereConditions) : void 0).orderBy(leaveBalanceTransactions.createdAt);
  ...
  async computeInitialLeaveBalances(orgId = 60, externalEmployeeData) {
    ...
    }).from(leaveBalanceTransactions).where(
      and(
        eq(leaveBalanceTransactions.orgId, orgId),
    ...
    for (const assignment of assignments) {
      ...
      if (importedEmployeeKeys.has(assignmentKey)) {
        ...
        if (existingBalance2) {
          ...
          const newTotalEntitlement = existingBalance2.totalEntitlement + configuredEntitlement;
          const newCurrentBalance = existingBalance2.currentBalance + configuredBalance;
  ...
  async syncPendingDeductionsForUser(userId, orgId) {
    ...
    try {
      ...
      await db.delete(leaveBalanceTransactions).where(and(
        eq(leaveBalanceTransactions.userId, userId),
        eq(leaveBalanceTransactions.orgId, orgId),
      ...
      for (const request of pendingRequests) {
        ...
        const existingPendingTransaction = await db.select().from(leaveBalanceTransactions).where(and(
          eq(leaveBalanceTransactions.userId, userId),
          eq(leaveBalanceTransactions.leaveVariantId, variant.id),
          // Use the actual variant ID we found
          eq(leaveBalanceTransactions.orgId, orgId),
          eq(leaveBalanceTransactions.transactionType, "pending_deduction"),
          like(leaveBalanceTransactions.description, `%Request ${request.id}%`)
...
init_schema();
init_schema();
import { eq as eq3, and as and3, desc as desc2, inArray as inArray2 } from "drizzle-orm";
...
async function handleCompOffTransfer(compOffRequest, orgId) {
  try {
    ...
    const existingBalance = await storage.getEmployeeLeaveBalances(compOffRequest.userId, (/* @__PURE__ */ new Date()).getFullYear(), orgId);
...
async function registerRoutes(app2) {
    ...
    try {
      ...
      if (userAssignments.length > 0 && existingBalances.length === 0) {
        ...
        try {
          ...
          if (jwtToken) {
            ...
            if (response.ok) {
              ...
              employeeData = data.data?.find((emp) => emp.user_id?.toString() === userId?.toString());
        ...
        for (const assignment of userAssignments) {
          const variant = leaveVariants2.find((v) => v.id === assignment.leaveVariantId);
  ...
  app2.post("/api/company", isAuthenticated, async (req, res) => {
    try {
      ...
      const validatedData = insertCompanySchema.parse({ ...req.body, orgId: parseInt(orgId) });
  ...
  app2.get("/api/leave-types", isAuthenticated, async (req, res) => {
    try {
      const orgId = req.headers["x-org-id"] || "60";
      const leaveTypes2 = await storage.getLeaveTypes(parseInt(orgId));
  ...
  app2.post("/api/roles", isAuthenticated, async (req, res) => {
    try {
      const orgId = req.headers["x-org-id"] || "60";
      const validatedData = insertRoleSchema.parse({ ...req.body, orgId: parseInt(orgId) });
  ...
  app2.delete("/api/users/:userId/roles/:roleId", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.get("/api/users/:userId/permissions", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.get("/api/employee-assignments/:variantId", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.get("/api/employee-assignments/pto/:variantId", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.get("/api/employee-assignments/comp-off-variant/:variantId", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.get("/api/employee-assignments", isAuthenticated, async (req, res) => {
    try {
      const orgId = req.headers["x-org-id"] || "60";
      const assignments = await storage.getEmployeeAssignments(parseInt(orgId));
      res.json(assignments);
    } catch (error) {
      console.error("Error fetching employee assignments:", error);
      res.status(500).json({ message: "Failed to fetch employee assignments" });
  ...
  app2.post("/api/employee-assignments/bulk", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
      let assignments;
      ...
      const validAssignments = assignments.filter((assignment) => assignment.userId && assignment.userId !== null);
  ...
  app2.get("/api/employee-leave-balances/all", isAuthenticated, async (req, res) => {
    try {
      const orgIdHeader = req.headers["x-org-id"];
      const orgId = parseInt(orgIdHeader) || 60;
      ...
      console.log(`\u{1F504} [API Route] Triggering bulk sync for pending deductions before HR report`);
  ...
  app2.get("/api/employee-leave-balances/:userId", isAuthenticated, async (req, res) => {
    try {
      const orgIdHeader = req.headers["x-org-id"];
      const orgId = parseInt(orgIdHeader) || 60;
      const userId = req.params.userId;
      ...
      console.log(`\u{1F4CA} [API] Returning ${balances.length} balances for user ${userId}`);
      if (balances.length > 0) {
        console.log(`\u{1F4CA} [API] Sample balance: userId=${balances[0].userId}, currentBalance=${balances[0].currentBalance}`);
      }
      res.json(balances);
    } catch (error) {
  ...
  app2.get("/api/leave-balance-transactions/all", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.get("/api/leave-balance-transactions/:userId", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
      const userId = req.params.userId;
  ...
  app2.post("/api/recalculate-prorata-balances", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.post("/api/fix-missing-balances", isAuthenticated, async (req, res) => {
    try {
      ...
      const assignments = await storage.getEmployeeAssignments(orgId);
      let fixedUsers = 0;
      const approvedRequests = leaveRequests2.filter((req2) => req2.status === "approved");
      for (const request of approvedRequests) {
        const existingBalances = await storage.getEmployeeLeaveBalances(request.userId, (/* @__PURE__ */ new Date()).getFullYear(), orgId);
        if (existingBalances.length === 0) {
          console.log(`[FixBalances] User ${request.userId} has approved request but no balances - creating...`);
          const userAssignments = assignments.filter((a) => a.userId === request.userId);
          for (const assignment of userAssignments) {
            const variant = leaveVariants2.find((v) => v.id === assignment.leaveVariantId);
            if (!variant || !variant.paidDaysInYear) continue;
            const currentYear = (/* @__PURE__ */ new Date()).getFullYear();
            ...
            const variantRequests = approvedRequests.filter(
              (req2) => req2.userId === request.userId && leaveVariants2.find((v) => v.id === assignment.leaveVariantId && v.leaveTypeId === req2.leaveTypeId)
            );
            const totalUsedDays = variantRequests.reduce((sum, req2) => {
  ...
  app2.post("/api/recalculate-leave-balances", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.post("/api/force-after-earning-transaction", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.post("/api/workflows", isAuthenticated, async (req, res) => {
    try {
      ...
      const validatedData = insertWorkflowSchema.parse({ ...req.body, orgId: parseInt(orgId) });
  ...
  app2.post("/api/comp-off-config", isAuthenticated, async (req, res) => {
    try {
      const orgId = req.headers["x-org-id"] || "60";
      const validatedData = insertCompOffConfigSchema.parse({ ...req.body, orgId: parseInt(orgId) });
  ...
  app2.post("/api/comp-off-variants", isAuthenticated, async (req, res) => {
    try {
      ...
      const validatedData = insertCompOffVariantSchema.parse({ ...req.body, orgId: parseInt(orgId) });
  ...
  app2.patch("/api/comp-off-variants/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const orgId = req.headers["x-org-id"] || "60";
      const variant = await storage.updateCompOffVariant(id, { ...req.body, orgId: parseInt(orgId) });
      res.json(variant);
    } catch (error) {
  ...
  app2.delete("/api/comp-off-variants/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app2.post("/api/pto-variants", isAuthenticated, async (req, res) => {
    try {
      const orgId = req.headers["x-org-id"] || "60";
      const validatedData = insertPTOVariantSchema.parse({ ...req.body, orgId: parseInt(orgId) });
  ...
  app2.patch("/api/pto-variants/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const orgId = req.headers["x-org-id"] || "60";
      const variant = await storage.updatePTOVariant(id, { ...req.body, orgId: parseInt(orgId) });
      res.json(variant);
    } catch (error) {
  ...
  app2.post("/api/pto-config", isAuthenticated, async (req, res) => {
    try {
      const orgId = req.headers["x-org-id"] || "60";
      const validatedData = insertPTOConfigSchema.parse({ ...req.body, orgId: parseInt(orgId) });
  ...
  app2.get("/api/leave-requests", isAuthenticated, async (req, res) => {
    try {
      const userId = req.query.userId;
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
      console.log("\u{1F50D} [Server] Leave requests query:", { userId, orgId });
      const requests = await storage.getLeaveRequests(userId, orgId);
      console.log("\u{1F4CA} [Server] Leave requests result:", {
    ...
    try {
      ...
      if (finalStatus === "pending") {
        try {
          ...
          const appliedVariant = leaveVariants2.find((v) => v.leaveTypeId === request.leaveTypeId);
      ...
      if (shouldDeductBalance || deductForPending) {
        try {
          ...
          const appliedVariant = leaveVariants2.find((v) => v.leaveTypeId === request.leaveTypeId);
          if (appliedVariant) {
            const balances = await storage.getEmployeeLeaveBalances(request.userId, (/* @__PURE__ */ new Date()).getFullYear(), orgId);
  ...
  app2.post("/api/leave-requests/:id/withdraw", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.post("/api/leave-requests/:id/approve", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const userId = req.user?.claims?.sub;
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
      ...
      if (request.status === "withdrawal_pending") {
        const updatedRequest2 = await storage.processWorkflowApproval(id, userId ?? "", orgId);
      ...
      if (updatedRequest.workflowStatus === "completed") {
        try {
          const leaveVariants2 = await storage.getLeaveVariants(orgId);
          let balances = await storage.getEmployeeLeaveBalances(request.userId, (/* @__PURE__ */ new Date()).getFullYear(), orgId);
          const assignments = await storage.getEmployeeAssignments(orgId);
          const userAssignments = assignments.filter((a) => a.userId === request.userId);
          ...
          if (createdAnyBalances) {
            balances = await storage.getEmployeeLeaveBalances(request.userId, (/* @__PURE__ */ new Date()).getFullYear(), orgId);
          }
          console.log(`[Approval] Available leave variants for leaveTypeId ${request.leaveTypeId}:`, leaveVariants2.filter((v) => v.leaveTypeId === request.leaveTypeId));
          console.log(`[Approval] User ${request.userId} balances:`, balances);
          let relevantBalance = null;
          let appliedVariant = null;
          for (const balance of balances) {
            const variant = leaveVariants2.find((v) => v.id === balance.leaveVariantId);
  ...
  app2.post("/api/leave-requests/:id/reject", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app2.post("/api/leave-variants", isAuthenticated, async (req, res) => {
    try {
      ...
      const variant = await storage.createLeaveVariant({ ...req.body, orgId: parseInt(orgId) });
      res.json(variant);
    } catch (error) {
  ...
  app2.post("/api/holidays", isAuthenticated, async (req, res) => {
    try {
      const orgId = req.headers["x-org-id"] || "60";
      const holiday = await storage.createHoliday({ ...req.body, orgId: parseInt(orgId) });
      res.json(holiday);
    } catch (error) {
  ...
  const findApplicableWorkflow = async (process2, subProcess, orgId) => {
    const workflows2 = await storage.getWorkflows(orgId);
    return workflows2.find((w) => w.process === process2 && w.subProcesses && w.subProcesses.includes(subProcess));
  };
  ...
  app2.patch("/api/comp-off-requests/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const orgId = req.headers["x-org-id"] || "60";
  ...
  app2.delete("/api/comp-off-requests/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app2.post("/api/comp-off-requests/:id/approve", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app2.post("/api/comp-off-requests/:id/reject", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app2.post("/api/pto-requests/:id/approve", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app2.post("/api/pto-requests/:id/reject", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app2.post("/api/upload-documents", isAuthenticated, documentUpload.array("documents", 5), async (req, res) => {
    try {
  ...
  app2.post("/api/import-leave-data/validate", isAuthenticated, upload.single("file"), async (req, res) => {
    try {
      ...
      rawData.forEach((row, index2) => {
        if (index2 < 10) {
          ...
          console.log(`[ExcelValidation] Row ${index2} [0]:`, row[0], typeof row[0]);
          console.log(`[ExcelValidation] Row ${index2} [1]:`, row[1], typeof row[1]);
          console.log(`[ExcelValidation] Row ${index2} [2]:`, row[2], typeof row[2]);
          console.log(`[ExcelValidation] Row ${index2} [3]:`, row[3], typeof row[3]);
        }
      });
      const jsonData = rawData.filter((row, index2) => {
        if (index2 < 3) {
          console.log(`[ExcelValidation] Skipping row ${index2} - header rows`);
          return false;
        }
        ...
        if (importType === "balances") {
          ...
          const endDate = processExcelDate2(row[4]);
      ...
      console.log(`[ExcelValidation] Available leave types: ${leaveTypes2.map((lt) => lt.name).join(", ")}`);
      ...
      for (let index2 = 0; index2 < jsonData.length; index2++) {
        ...
        if (importType === "balances") {
          ...
          const validHalfDayValues = ["TRUE", "FALSE", true, false, "", null, void 0];
        ...
        else if (row.LeaveType === "Privelege Leave") mappedLeaveType = "Privilege Leave";
        ...
        if (mappedLeaveType && !leaveTypeNames.includes(mappedLeaveType.toLowerCase())) {
          validationErrors.push(`Row ${rowNum}: Leave Type "${row.LeaveType}" not found. Available types: ${leaveTypes2.map((lt) => lt.name).join(", ")}`);
        }
      ...
      res.status(500).json({ message: "Failed to validate file: " + error.message });
    }
  });
  app2.post("/api/import-leave-data/execute", isAuthenticated, upload.single("file"), async (req, res) => {
    try {
      let excelDateToJSDate2 = function(serial2) {
        const epochDiff = 25569;
        const msPerDay = 864e5;
      ...
      for (const row of jsonData) {
        ...
        if (importType === "balances") {
          ...
          console.log(`[ExcelImport] Available leave types:`, leaveTypes2.map((lt) => lt.name));
          const leaveType = leaveTypes2.find((lt) => lt.name.toLowerCase() === mappedLeaveType.toLowerCase());
          if (!leaveType) {
          ...
          const leaveVariants2 = await storage.getLeaveVariants(orgId);
          console.log(`[ExcelImport] Available leave variants:`, leaveVariants2.map((v) => ({ id: v.id, leaveTypeId: v.leaveTypeId, name: v.leaveTypeName })));
          const variant = leaveVariants2.find((v) => v.leaveTypeId === leaveType.id);
          if (!variant) {
          ...
          try {
            ...
            await storage.upsertLeaveBalance({
              ...
              // Annual entitlement in full days
              currentBalance,
              // Available balance in full days
              usedBalance: totalUsed,
              // Used balance in full days
              carryForward: 0,
            ...
            if (configuredEntitlement > 0) {
              transactions.push({
                ...
                leaveVariantId: variant.id,
                transactionType: "grant",
                amount: configuredEntitlement,
                // Configured entitlement in full days
            ...
            if (importedEntitlement > 0) {
              transactions.push({
                ...
                leaveVariantId: variant.id,
                transactionType: "grant",
                amount: importedEntitlement,
                // Imported balance in full days
                balanceAfter: annualEntitlement,
                // Total after both grants
            ...
            if (availed > 0) {
              transactions.push({
                ...
                transactionType: "deduction",
                amount: -availed,
                // Negative for deduction in full days
        ...
        for (const assignment of userAssignments) {
          ...
          if (calculationMethod === "auto" && employee?.date_of_joining) {
            ...
            if (variant.grantLeaves === "in_advance") {
              ...
              const monthsWorked = Math.floor((currentDate.getTime() - joiningDate.getTime()) / (30.44 * 24 * 60 * 60 * 1e3));
              const monthlyAllocation = entitlement / 12;
              calculatedBalance = Math.floor(monthsWorked * monthlyAllocation);
            }
          } else {
  ...
  app2.post("/api/process-time-based-approvals", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.get("/api/pending-time-based-approvals", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.post("/api/migrate-employee-data", isAuthenticated, async (req, res) => {
    try {
      ...
      const { migrateEmployeeData: migrateEmployeeData2 } = await init_employeeMapping().then(() => employeeMapping_exports);
  ...
  app2.get("/api/collaborative-leave-settings", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.put("/api/collaborative-leave-settings", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.patch("/api/collaborative-leave-settings", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
      const updateData = { ...req.body, updatedAt: /* @__PURE__ */ new Date() };
      const [updated] = await db.update(collaborativeLeaveSettingsEnhanced).set(updateData).where(eq3(collaborativeLeaveSettingsEnhanced.orgId, orgId)).returning();
  ...
  app2.get("/api/leave-requests/:leaveRequestId/tasks", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
      const leaveRequestId = parseInt(req.params.leaveRequestId);
      const tasks = await db.select().from(leaveTaskAssigneesEnhanced).where(and3(
  ...
  app2.get("/api/collaborative-tasks", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
      const allTasks = await db.select().from(leaveTaskAssigneesEnhanced).leftJoin(leaveRequests, eq3(leaveTaskAssigneesEnhanced.leaveRequestId, leaveRequests.id)).where(eq3(leaveTaskAssigneesEnhanced.orgId, orgId)).orderBy(desc2(leaveTaskAssigneesEnhanced.createdAt));
  ...
  app2.post("/api/leave-requests/:leaveRequestId/tasks", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
      const leaveRequestId = parseInt(req.params.leaveRequestId);
  ...
  app2.patch("/api/tasks/:taskId/status", isAuthenticated, async (req, res) => {
    try {
      ...
      const userId = req.user?.claims?.sub;
      const [task] = await db.select().from(leaveTaskAssigneesEnhanced).where(and3(
  ...
  app2.post("/api/leave-requests/:leaveRequestId/closure-report", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
      const leaveRequestId = parseInt(req.params.leaveRequestId);
  ...
  app2.patch("/api/closure-reports/:reportId/review", isAuthenticated, async (req, res) => {
    try {
      const orgId = parseInt(req.headers["x-org-id"]) || 60;
  ...
  app2.get("/api/leave-requests/:leaveRequestId/tasks", isAuthenticated, async (req, res) => {
    try {
      ...
      const tasks = await db.select().from(leaveTaskAssigneesEnhanced).where(and3(
        eq3(leaveTaskAssigneesEnhanced.leaveRequestId, parseInt(leaveRequestId)),
        eq3(leaveTaskAssigneesEnhanced.orgId, orgId)
      ));
      res.json(tasks);
    } catch (error) {
  ...
  app2.post("/api/leave-requests/:leaveRequestId/tasks", isAuthenticated, async (req, res) => {
    try {
      ...
      const { leaveRequestId } = req.params;
      const { tasks } = req.body;
      console.log("\u{1F536} Creating tasks with assigneeUserId data:", tasks.map((t) => ({
        name: t.assigneeName,
  ...
  app2.get("/api/tasks/assigned-by-me/:userId", async (req, res) => {
    try {
      const { userId } = req.params;
      const orgId = req.headers["x-org-id"] ? parseInt(req.headers["x-org-id"]) : 60;
      const userLeaveRequests = await db.select({ id: leaveRequests.id }).from(leaveRequests).where(and3(
        eq3(leaveRequests.userId, userId),
        eq3(leaveRequests.orgId, orgId)
      ));
  ...
  app2.get("/api/leave-requests/:leaveRequestId/closure-report", isAuthenticated, async (req, res) => {
    try {
      const orgId = req.headers["x-org-id"] ? parseInt(req.headers["x-org-id"]) : 60;
      const { leaveRequestId } = req.params;
      const report = await db.select().from(leaveClosureReportsEnhanced).where(and3(
        eq3(leaveClosureReportsEnhanced.leaveRequestId, parseInt(leaveRequestId)),
  ...
  app2.post("/api/leave-requests/:leaveRequestId/closure-report", isAuthenticated, async (req, res) => {
    try {
      ...
      const existingReport = await db.select().from(leaveClosureReportsEnhanced).where(and3(
        eq3(leaveClosureReportsEnhanced.leaveRequestId, parseInt(leaveRequestId)),
        eq3(leaveClosureReportsEnhanced.orgId, orgId)
      )).limit(1);
